    ...
    if(nonEmptyNumber(arr) == SIZE){
     ...
     if(/*!haveCombined &&*/ arr[1] == arr[2]&&arr[1]&&arr[2]) {
      ...
      if(/*!haveCombined &&*/ arr[0] == arr[1]&&arr[0]&&arr[1]){
        ...
        bool haveCombined = false; // 声明一个变量来跟踪是否已经进行了合并操作

    // 向上移动非零元素
    PushtoTop(arr);

    // 从数组的第二个元素开始，逐个检查并合并相邻且相同的元素
    for (int i = 1; i < SIZE; i++) {
        if (arr[i] == arr[i - 1] && arr[i] != 0 && !haveCombined) { // 检查是否相邻且相同的非零元素
            CombineValue(arr, i, i - 1); // 合并相邻元素
             calculateScore(arr[i]);
            haveCombined = true; // 标记已经进行了合并操作
            PushtoTop(arr); // 再次向上移动非零元素
        } else {
            haveCombined = false; // 如果没有进行合并操作，重置标志
        }
    }
    }
}  
